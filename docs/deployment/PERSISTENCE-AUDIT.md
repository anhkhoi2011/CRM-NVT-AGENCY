...
Chạy npm test: 43 kiểm tra tự động đã qua (transaction mô phỏng, phân quyền, replay, xung đột, phục hồi bản nháp, phân quyền tài khoản, API đăng ký, dựng view và luồng webhook). Kiểm tra cú pháp JavaScript và git diff --check cũng được thực hiện. Test dùng pool/DOM giả lập, không chứng minh hành vi thực tế của MySQL, trình duyệt, cPanel hoặc reverse proxy.
