...
(() => {
  ...
  function renderDistributionWeights(){
    ...
    let blocks=managers.map(manager=>block(manager.name||'Manager', 'MANAGER - '+(manager.teamId||'Quan ly tuyen'), row(manager,'MANAGER',0)+directSales(manager.id).map(sale=>row(sale,'SALE',1)).join('')+byManager(manager.id).map(leader=>row(leader,'LEADER',1)+byLeader(leader.id).map(sale=>row(sale,'SALE',2)).join('')).join(''),managerCycle(manager))).join('');
