...
(() => {
  ...
  function attendanceDetailMarkup(memberId,selectedMonth=''){
    ...
    const monthOptions=Array.from({length:3},(_,offset)=>{const d=new Date(today+'T00:00:00Z');d.setUTCDate(1);d.setUTCMonth(d.getUTCMonth()-offset);const value=d.toISOString().slice(0,7),label=d.toLocaleDateString('vi-VN',{month:'long',year:'numeric',timeZone:'UTC'});return `<option value="${value}" ${value===monthValue?'selected':''}>${esc(label)}</option>`;}).join('');
  ...
  function renderDistributionWeights(){
    ...
    const globalCycle=()=>{
      ...
      const roundRoster=[...managers.map(member=>({...member,role:'SALE',managerRecipient:true})),...leaders.map(member=>({...member,role:'SALE',teamLeaderRecipient:true})),...sales];
      ...
      const queuedMarkup=queuedRounds.map((round,index)=>{
        const enabled=new Set(Array.isArray(round.enabledSaleIds)?round.enabledSaleIds:roundRoster.map(person=>person.id));
        const hasRoundMemberMatch=roundRoster.some(person=>enabled.has(person.id));
        const eligible=roundRoster.filter(person=>hasRoundMemberMatch?enabled.has(person.id):true);
        const sequence=eligible.filter(person=>weightOf(round.weights?.[person.id])>0).flatMap(person=>Array.from({length:weightOf(round.weights?.[person.id])},()=>person));
