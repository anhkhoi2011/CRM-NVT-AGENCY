...
(() => {
  ...
  function renderDistributionWeights(){
    ...
    const globalCycle=()=>{
      ...
      return people.length?cycleMarkup(cycleSummary(people,[config],'global'),'WEIGHTED'):'';
    };
    ...
    host.innerHTML='<section class="panel" style="background:var(--bg-surface);border:1px solid var(--border);border-radius:12px;padding:22px;box-shadow:var(--shadow-sm)"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:14px;margin-bottom:18px;flex-wrap:wrap"><div><b style="font-size:15px;color:var(--text-main);display:block">Chinh sua ty trong</b><div style="font-size:12px;color:var(--text-muted);margin-top:3px">Cau hinh Manager, Leader va Sale trong cung mot he thong nhan data.</div></div><span class="chip">'+members.length+' nhan su</span></div><div class="distribution-global-cycle"><div class="distribution-global-cycle-title"><b>Vong phan data toan he thong</b><small>Manager, Leader va Sale dung chung mot vong; ty trong cao se co nhieu luot hon.</small></div>'+globalCycle()+'</div><div class="distribution-tree-list">'+(blocks||'<div class="empty"><b>Chua co Manager, Leader hoac Sale</b><span>Hay phan bo nhan su tai muc Doi ngu.</span></div>')+'</div></section>';
  ...
  function renderManagementDataTable(role){
    ...
    const visible=role==='ADMIN'?data.customers:data.customers.filter(c=>role==='MANAGER'?(c.ownerId===currentId||c.managerId===currentId||leaderIds.has(c.leaderId)||salesIds.has(c.saleId)):(c.ownerId===currentId||c.leaderId===currentId||salesIds.has(c.saleId)));
    ...
    const rows=visible.map(customer=>{const offer=latestOffer(customer),returned=returns.get(customer.id),type=returned?'DATA ĐIỀN LẠI FORM':offer?.status==='EXPIRED'&&!customer.saleId?'DATA TRẢ VỀ':'DATA MỚI',at=returned?.at||(type==='DATA TRẢ VỀ'?(offer?.resolvedAt||offer?.offeredAt):customer.createdAt);return {customer,offer,type,at:at||customer.createdAt};});
    ...
    filtered.forEach(row=>{const assigned=row.customer.saleId||row.offer?.saleId||row.customer.leaderId||row.customer.managerId;if(assigned&&!recipients.some(member=>member.id===assigned)){const member=members.find(item=>item.id===assigned);if(member)recipients.push(member);}});
    ...
    const statusFor=(customer,offer,type)=>type==='DATA TRẢ VỀ'&&offer?.status==='PENDING'?'Đã phân lại · chờ Sale nhận':offer?.status==='PENDING'?'Chờ Sale nhận':offer?.status==='EXPIRED'&&!customer.saleId?'Data Sale không nhận':customer.saleId||offer?.saleId?'Sale đã nhận':customer.managerId?'Đã phân Manager':customer.leaderId?'Đã phân Leader':'Chưa chọn Sale';
    ...
    renderDataQueuePagination(table,filtered.length);text('sidebarDataBadge',filtered.length);text('dataTabCountBadge',filtered.length);
  }

  const updateDataDateFilterSummary=()=>{
    const summary=q('#dataDateFilterSummary');if(!summary)return;
    const from=String(dataQueueDateFrom||'').slice(0,10),to=String(dataQueueDateTo||'').slice(0,10);
    summary.textContent=!from&&!to?'Chưa lọc theo ngày':from&&to?'Đang lọc từ '+fmtDate(from)+' đến '+fmtDate(to):'Đang lọc ngày '+fmtDate(from||to);
  };
