...
(() => {
  ...
  const renderScopedQueueUnified = role => {
    ...
    const staff=canAssign?members.filter(m=>m.role==='SALE'&&m.active!==false&&(leaderIds.has(m.leaderId)||(role==='MANAGER'&&m.managerId===currentId))).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'vi')):[];
