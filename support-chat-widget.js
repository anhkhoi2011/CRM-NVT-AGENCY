...
(() => {
  ...
  function ensureStyle() {
    ...
    style.textContent += `#${widgetId} .support-launch{font-size:0}#${widgetId} .support-launch::before{content:'\\1F3A7';display:block;font-size:24px;line-height:1}#${widgetId} .support-launch-badge{font-size:10px}`;
  ...
  function launcherMarkup() {
    ...
    return `<button type="button" class="support-launch" data-support-toggle aria-label="Mở hỗ trợ nội bộ">?${unread ? `<span class="support-launch-badge">${unread > 99 ? '99+' : unread}</span>` : ''}</button>`;
  }
